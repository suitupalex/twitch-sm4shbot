...
function chatEmit(command, challenger) {
  client.emit(
    'chat'
  , `#${TEST_CHANNEL}`
...
tap.test('should lose the match', function matchLoseTest(t) {
  ...
  t.equal(match.wins, 0)
  t.equal(match.losses, 2)
  t.notOk(match.checkRecord())
  
  chatEmit('loss')

  t.equal(match.wins, 0)
  t.equal(match.losses, 3)
  t.ok(match.checkRecord())
...
tap.test('should list challengers', function listTest(t) {
  ...
  t.ok(new Date().valueOf() - client.lastListTime < 1000)
...
tap.test('should not list challengers', function notListTest(t) {
  function delayList() {
    ...
    t.notOk(new Date().valueOf() - client.lastListTIme < 1000)

    t.end()
  }

  setTimeout(delayList, 1000)
})
