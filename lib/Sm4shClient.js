...
function Sm4shClient(config) {
  ...
  config._sb = config._sb || {}

  Client.call(this, config)

  this._sb = {}
  this._sb.store = config._sb.store
  // Use an object because we care more about speed than memory.
  this._sb.channels = config._sb.channels || {}
...
Sm4shClient.prototype._handleJoin = function _handleJoin(rawChannel, user) {
  ...
  if (this._sb.store) {
    this._sb.store.sadd('_sb-channels', channelname)
  }
...
, isSelf
  ...
  if (
     isSelf
  || !message
  || (sm4sh !== '!sm4sh ' && sm4sh !== '!smash ' && sm4sh !== '!enter ')
  ) {
    return
  }
  ...
  if (
     (username === channelname || channel.trusted[username] === 'admin')
  && sm4sh !== '!enter '
  ) {
    switch (command) {
      case 'on':
...
Sm4shClient.prototype._updateSocket = function _updateSocket() {
  ...
  const channels = self._sb.channels
  ...
  const update = keys.map(function keyMapper(key) {
    ...
    if (self._sb.store) {
      self._sb.store.set(`_sb-channel-${key}`, JSON.stringify(info))
    }

    return info
  })

  self.log.info('Sending update:', update)
  self.socket.update(update)
}
